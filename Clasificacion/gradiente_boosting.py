...
def graficar(df, res, nombre_grupo, modo, umbral):
    ...
    fig.suptitle(f"{nombre_grupo}  ·  Gradient Boosting  ·  {modo}",
    ...
    ax.barh(features, importancias, color=color_m, edgecolor="white")
    ax.axvline(0, color="black", linewidth=0.8)
    ax.set_xlabel("Importancia")
    ax.set_title("Importancia de variables")
    rng = max(importancias) * 0.03
    for i, v in enumerate(importancias):
        ax.text(v + rng, i, f"{v:.3f}", va="center", fontsize=9)
    ...
    slug = f"{nombre_grupo}_gradient_boosting".replace(" ", "_").lower()
