...
# ══════════════════════════════════════════════════════════════════
# SIMULACIÓN
# Riesgo de reprobación: depende principalmente de
# calificación baja y entregas de tareas bajas.
# ══════════════════════════════════════════════════════════════════
def simular_datos(n: int, seed: int = 42) -> pd.DataFrame:
    ...
    # Alumnos con cal >= 75 rara vez reprueban
    evento[cal_final >= 75] = rng.choice(
...
def graficar(df_orig, df_norm, cph, nombre_grupo):
    n        = len(df_orig)
    c_ev     = "#F44336"   # rojo  — reprobó
    c_noev   = "#4CAF50"   # verde — no reprobó
    c_cur    = "#E53935"
    ...
    fig.suptitle(f"{nombre_grupo}  ·  Cox PH  ·  Reprobación",
    ...
    ax.hist(df_orig.loc[df_orig["evento"] == 0, "cal_final"],
            bins=12, alpha=0.7, color=c_noev, label="No reprobó")
    ax.hist(df_orig.loc[df_orig["evento"] == 1, "cal_final"],
            bins=12, alpha=0.7, color=c_ev,   label="Reprobó")
    ax.set_title("Cal. final por estado")
    ax.set_xlabel("Calificación"); ax.set_ylabel("Alumnos")
    ax.legend(fontsize=8)

    ax = fig.add_subplot(gs[0, 1])
    conteo = df_orig["evento"].value_counts().sort_index()
    ax.pie(conteo, labels=["No reprobó", "Reprobó"],
           autopct="%1.1f%%",
           colors=[c_noev, c_ev], startangle=90,
           wedgeprops={"edgecolor": "white", "linewidth": 1.2})
    ax.set_title("Proporción reprobación")

    ax = fig.add_subplot(gs[0, 2])
    ax.hist(df_orig.loc[df_orig["evento"] == 1, "tiempo"],
            bins=16, color=c_ev,   alpha=0.7, label="Reprobó")
    ax.hist(df_orig.loc[df_orig["evento"] == 0, "tiempo"],
            bins=16, color=c_noev, alpha=0.7, label="No reprobó")
    ax.set_title("Semanas hasta reprobar")
    ...
    mask_alto = df_orig["cal_final"] >= 75
    if mask_alto.sum() > 1:
        KaplanMeierFitter().fit(
            df_orig.loc[mask_alto, "tiempo"],
            event_observed=df_orig.loc[mask_alto, "evento"],
            label="Cal. ≥ 75"
        ).plot_survival_function(ax=ax, color=c_noev, ci_show=False)

    mask_bajo = df_orig["cal_final"] < 65
    if mask_bajo.sum() > 1:
        KaplanMeierFitter().fit(
            df_orig.loc[mask_bajo, "tiempo"],
            event_observed=df_orig.loc[mask_bajo, "evento"],
            label="Cal. < 65"
        ).plot_survival_function(ax=ax, color=c_ev, ci_show=False)

    ax.set_title("Kaplan-Meier — P(no reprobar) por semana")
    ax.set_xlabel("Semana"); ax.set_ylabel("P(no reprobar)")
    ax.set_ylim(0, 1.05)
    ...
    ax.set_title("Riesgo de reprobación por alumno\n"
                 "(rojo=reprobó, verde=no reprobó)")
    ...
    ax.set_title("P(no reprobar) predicha por perfil típico")
    ax.set_xlabel("Semana"); ax.set_ylabel("P(no reprobar)")
    ax.set_ylim(0, 1.05)
    ax.legend(fontsize=9)

    slug = f"{nombre_grupo}_cox_reprobacion".replace(" ", "_").lower()
...
def analizar_grupo(df: pd.DataFrame, nombre_grupo: str = "Grupo"):
    ...
    print(f"\n{'═'*50}")
    print(f"  {nombre_grupo} — Cox PH — Reprobación")
    print(f"{'═'*50}")
    cph.print_summary()
